...
struct semaphore *CatMouseWait;
  ...
  struct lock *SpeciesLock;
  struct cv *CatsWait, *MiceWait;
  bool *BowlInUse;
  volatile int EatingCatsCount = 0;
  volatile int EatingMiceCount = 0;
#endif /* OPT_A1 */
               ...
               unsigned long catnumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
    ...
    #if OPT_A1
      lock_acquire(SpeciesLock);
        while(EatingMiceCount != 0 || EatingCatsCount >= NumBowls){
          cv_wait(CatsWait, SpeciesLock);
        }

        bowl = ((unsigned int)random() % NumBowls);
        struct lock *CurBowl = BowlLocks[bowl];
        while (BowlInUse[bowl]){
          bowl = (bowl + 1) % NumBowls;
          CurBowl = BowlLocks[bowl];
        }

        lock_acquire(CurBowl);
          BowlInUse[bowl] = true;
          EatingCatsCount++;
      lock_release(SpeciesLock);
          cat_eat(bowl+1, CatEatTime);
      lock_acquire(SpeciesLock);
          EatingCatsCount--;
        BowlInUse[bowl] = false;
        lock_release(CurBowl);
        cv_broadcast(MiceWait, SpeciesLock);
      lock_release(SpeciesLock);
    #else
          ...
          unsigned long mousenumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
    ...
    #if OPT_A1
      lock_acquire(SpeciesLock);
        while(EatingCatsCount != 0 || EatingMiceCount>=NumBowls){
          cv_wait(MiceWait, SpeciesLock);
        }
        ...
        lock_acquire(CurBowl);
          ...
          EatingMiceCount++;
      lock_release(SpeciesLock);
          mouse_eat(bowl+1, MouseEatTime);
      lock_acquire(SpeciesLock);
          EatingMiceCount--;
        BowlInUse[bowl] = false;
        lock_release(CurBowl);
        cv_broadcast(CatsWait, SpeciesLock);
      lock_release(SpeciesLock);
    #else
      /* legal bowl numbers range from 1 to NumBowls */
      bowl = ((unsigned int)random() % NumBowls) + 1;
         ...
         char ** args)
{
  ...
  #if OPT_A1
    /*
     * Initialize BowlLocks, SpeciesLock, CatsWait, and MiceWait.
     */
    BowlLocks = kmalloc(sizeof (struct lock *) * NumBowls);
    BowlInUse = kmalloc(sizeof (bool) * NumBowls);
    for (index = 0; index < NumBowls; index++){
      BowlLocks[index] = lock_create("bowl");
      BowlInUse[index] = false;
    }

    SpeciesLock = lock_create("SpeciesLock");

    CatsWait = cv_create("CatsWait");

    MiceWait = cv_create("MiceWait");
  #endif /* OPT_A1 */
